...
public class scr_FactoryMini2DMapController : MonoBehaviour
{
    ...
    [SerializeField, HideInInspector] private RectTransform robotMarker;
    [SerializeField, HideInInspector] private RectTransform headingMarker;
    [SerializeField, HideInInspector] private RectTransform headingArrow;
    ...
    [SerializeField, HideInInspector] private GameObject eventMarker;
    [SerializeField, HideInInspector] private Image robotMarkerImage;
    ...
    [SerializeField, HideInInspector] private Image eventMarkerImage;
    ...
    [SerializeField] private float miniMapDisplayScale = 4f;
    ...
    [SerializeField] private float headingArrowSize = 16f;
    [SerializeField] private float headingArrowOffset = 8f;
    ...
    private readonly Dictionary<string, RobotMiniMapState> statesByRobot = new();
    ...
    private static readonly ProfilerMarker MiniMapUpdateMarker = new("ControlTower.Map.MiniMap.Update");

    public bool UseMapDisplayScale => useMapDisplayScale;
    public float MiniMapDisplayScale => miniMapDisplayScale;
    ...
    private struct RobotMiniMapState
    {
        public float X;
        public float Y;
    ...
    {
        RobotMiniMapState state = statesByRobot.TryGetValue(robotId, out RobotMiniMapState previousState)
            ? previousState
            : default;
    ...
    private bool SyncAllRobotMarkersToLatestPoseImmediate()
    {
        ...
        for (int robotId = 1; robotId <= 3; robotId++)
        {
            ...
            if (uiManager.TryGetRobotState(robotName, out float robotX, out float robotY, out float robotHeading, out string robotStatus))
            {
                float robotReceiveTime = uiManager.TryGetRobotPoseReceiveTime(robotName, out float robotCachedReceiveTime)
                    ? robotCachedReceiveTime
                    : Time.unscaledTime;
                statesByRobot[robotName] = new RobotMiniMapState
                {
                    X = robotX,
                    Y = robotY,
                    Heading = robotHeading,
                    Status = robotStatus,
                    LastValidPoseReceiveTime = robotReceiveTime,
                    HasValidPose = true
                };
    ...
    private void UpdateMarkerPosition(string robotId, bool markPoseReceived, bool immediate = false)
    {
        ...
        if (!statesByRobot.TryGetValue(robotId, out RobotMiniMapState state))
        {
            return;
        }
        ...
        {
            Debug.Log(
                $"[TB3-03 Alignment MiniMap] rawPose=({state.X:F3},{state.Y:F3},{state.Heading:F3}) " +
    ...
    public void SetIncidentMarker(string incidentType, float rosX, float rosY, bool visible)
    {
        ...
        if (TryConvertRosToMapPosition(rosX, rosY, marker, out _, out _, out Vector2 clampedUi, out _, out _, out _, out _, out _, out _))
        {
            marker.anchoredPosition = ConvertMapLocalToMarkerParentLocal(clampedUi, marker);
    ...
    {
        RectTransform headingArrow = GetHeadingArrow(robotId);
        if (headingArrow == null)
        {
            return;
        }
        ...
        if (rotatedMarkerImage != null && headingArrow.IsChildOf(rotatedMarkerImage) &&
            headingArrow.parent != null)
        {
            RectTransform marker = GetMarker(robotId);
            ...
            headingArrow.localRotation =
                Quaternion.Inverse(headingArrow.parent.rotation) * desiredWorldRotation;
        }
        else
        {
            headingArrow.localRotation = Quaternion.Euler(0f, 0f, calibratedYawDeg);
        }
    }
        ...
        if (stateChanged)
        {
            ...
            if (logCoordinateDebug)
            {
                ...
                Vector3 baseScale = visualBaselinesByRobot.TryGetValue(robotId, out RobotMarkerVisualBaseline baseline)
                    ? baseline.BodyScale
                    : currentScale;
                Debug.Log($"[MiniMap] {robotId} visual={nextState} scale={currentScale} baseScale={baseScale}");
            }
        }
    ...
    private bool ShouldShowRobotMarker(RobotMiniMapState state)
    {
    ...
    {
        if (!statesByRobot.TryGetValue(robotId, out RobotMiniMapState state) || !state.HasValidPose)
        {
    ...
    private void ResolveReferences()
    {
        ...
        mapArea ??= FindRectTransform("Image_Mini2DMapArea");
        markerTb3_01 ??= FindRectTransform("Image_Mini2DMapTB3_01");
        markerTb3_02 ??= FindRectTransform("Image_Mini2DMapTB3_02");
        markerTb3_03 ??= FindRectTransform("Image_Mini2DMapTB3_03");
        headingArrowTb3_01 ??= FindChildRectTransform(markerTb3_01, "HeadingArrow_MiniMap_TB3_01");
        headingArrowTb3_02 ??= FindChildRectTransform(markerTb3_02, "HeadingArrow_MiniMap_TB3_02");
        headingArrowTb3_03 ??= FindChildRectTransform(markerTb3_03, "HeadingArrow_MiniMap_TB3_03");
        markerNoHelmet ??= FindRectTransform("Image_Mini2DMapNoHelmetMarker");
        markerFall ??= FindRectTransform("Image_Mini2DMapFallMarker");
        markerFire ??= FindRectTransform("Image_Mini2DMapFireMarker");
        markerImageTb3_01 ??= markerTb3_01 != null ? markerTb3_01.GetComponent<Image>() : null;
        markerImageTb3_02 ??= markerTb3_02 != null ? markerTb3_02.GetComponent<Image>() : null;
        markerImageTb3_03 ??= markerTb3_03 != null ? markerTb3_03.GetComponent<Image>() : null;
        if (robotMarker != null)
        {
            robotMarker.gameObject.SetActive(false);
        }
