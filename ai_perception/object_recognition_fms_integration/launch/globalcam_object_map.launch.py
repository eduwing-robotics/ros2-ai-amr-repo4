...
def generate_launch_description():
    ...
    calibration_path = Path(robot_face_dir) / "config/globalcam_perspective_calibration.json"
    try:
        perspective_calibration = json.loads(calibration_path.read_text())
    except (OSError, json.JSONDecodeError):
        perspective_calibration = {}
    default_safety_model = (
        f"{robot_face_dir}/models/safety_continue45_plus_nohelmet_e10_best.pt"
    )
    default_turtlebot_model = (
        f"{robot_face_dir}/models/turtlebot_edge_aug_plus70ep_best.pt"
    )
    ...
    alert_display_sec = LaunchConfiguration("alert_display_sec")
    ...
    fall_turtlebot_overlap_threshold = LaunchConfiguration("fall_turtlebot_overlap_threshold")
    ...
    perspective_calibration_mode = LaunchConfiguration("perspective_calibration_mode")
    perspective_preview = LaunchConfiguration("perspective_preview")
    perspective_preview_width = LaunchConfiguration("perspective_preview_width")
    perspective_preview_height = LaunchConfiguration("perspective_preview_height")
    map_image_corners = LaunchConfiguration("map_image_corners")
    show_map_point = LaunchConfiguration("show_map_point")
    show_turtlebot_goals = LaunchConfiguration("show_turtlebot_goals")
    map_point_x = LaunchConfiguration("map_point_x")
    map_point_y = LaunchConfiguration("map_point_y")
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("alert_display_sec", default_value="5.0"),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument("turtlebot_goal_offset_x", default_value="0.6"),
            DeclareLaunchArgument(
                "annotated_topic",
            ...
            DeclareLaunchArgument("perspective_calibration_mode", default_value="false"),
            DeclareLaunchArgument("perspective_preview", default_value="false"),
            DeclareLaunchArgument("perspective_preview_width", default_value="900"),
            DeclareLaunchArgument("perspective_preview_height", default_value="900"),
            DeclareLaunchArgument(
                "map_image_corners",
                default_value=str(perspective_calibration.get("map_image_corners", "")),
            ),
            DeclareLaunchArgument("show_map_point", default_value="false"),
            DeclareLaunchArgument("show_turtlebot_goals", default_value="true"),
            DeclareLaunchArgument("map_point_x", default_value="0.0"),
            DeclareLaunchArgument("map_point_y", default_value="0.0"),
            DeclareLaunchArgument("map_points", default_value=""),
            DeclareLaunchArgument("safety_model_path", default_value=default_safety_model),
            DeclareLaunchArgument("person_model_path", default_value=f"{robot_face_dir}/models/yolo11n.pt"),
            ...
            DeclareLaunchArgument("turtlebot_fps", default_value="4"),
            DeclareLaunchArgument("fall_turtlebot_overlap_threshold", default_value="0.7"),
            DeclareLaunchArgument("proximity_enter_distance", default_value="0.4"),
            DeclareLaunchArgument("proximity_exit_distance", default_value="0.5"),
            ...
            ExecuteProcess(
                cmd=[
                    ...
                    "--alert-display-sec",
                    alert_display_sec,
                    "--show-turtlebot-goals",
                    show_turtlebot_goals,
                    "--image-qos-depth",
                    image_qos_depth,
                    "--show-debug-overlay",
                    show_debug_overlay,
                    "--publish-annotated",
                    publish_annotated,
                    "--perspective-calibration-mode",
                    perspective_calibration_mode,
                    "--perspective-preview",
                    perspective_preview,
                    "--perspective-preview-width",
                    perspective_preview_width,
                    "--perspective-preview-height",
                    perspective_preview_height,
                    "--map-image-corners",
                    map_image_corners,
                    "--show-map-point",
                    show_map_point,
            ...
            ExecuteProcess(
                cmd=[
                    ...
                    fall_turtlebot_overlap_threshold,
