...
YOLO_TEST_DIR = Path(os.environ.get("YOLO_TEST_DIR", PROJECT_ROOT / "models"))
...
def find_latest_turtlebot_model() -> Path:
    ...
    for root in TURTLEBOT_MODEL_SEARCH_ROOTS:
        ...
        for path in root.rglob("*.pt"):
            ...
            if any(keyword in model_name for keyword in TURTLEBOT_MODEL_EXCLUDE_KEYWORDS):
                continue
            if "turtlebot" in model_name or "tb3" in model_name or "burger" in model_name:
...
class TurtlebotProximityDetector:
    def __init__(self, model_path: str, confidence: float, imgsz: int, device: str):
        try:
            ...
            raise RuntimeError(
                "Turtlebot proximity detector requires ultralytics and torch in the Python environment."
            ) from exc
...
def parse_args():
    ...
    parser.add_argument("--proximity-enter-distance", type=float, default=0.3)
    parser.add_argument("--proximity-exit-distance", type=float, default=0.4)
