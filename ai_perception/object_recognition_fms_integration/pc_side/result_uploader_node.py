...
class ResultUploader(Node):
    ...
    def active_session_url(self):
        return self.args.server_url.rstrip("/") + self.args.session_path

    def fetch_active_session(self):
        if not self.args.server_url or not self.args.session_path:
            return None
        now = time.monotonic()
...
def parse_args():
    ...
    parser.add_argument("--session-path", default="/api/attendance/session")
    parser.add_argument("--token", default="")
    parser.add_argument("--queue-path", default=str(nested_config(config, "uploader.queue_path", "/home/gyul/robot_face_system/data/pending_events.sqlite")))
    parser.add_argument("--local-log-path", default=str(nested_config(config, "uploader.local_log_path", "/home/gyul/robot_face_system/data/uploader_events.jsonl")))
