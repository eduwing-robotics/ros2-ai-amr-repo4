...
DEFAULT_DETECTOR_MODEL = "/home/yr/perception_server_bundle_final/models/hardhat_turtlebot_best.pt"
...
class SafetyIdentityNode(Node):
    def __init__(self, args):
        ...
        self.camera_sources = build_camera_sources(args)
        self.last_msg_at_by_source: dict[str, float] = {}
        ...
        for source in self.camera_sources:
            ...
            self.image_subscriptions.append(
                self.create_subscription(
                    msg_type,
                    source.topic,
                    self._make_image_callback(source),
                    qos,
                )
            )
            if args.publish_annotated:
                self.annotated_pubs[source.image_source] = self.create_publisher(
                    Image, source.annotated_topic, qos
                )
            self.get_logger().info(
                f"Listening {source.image_source}: {source.topic} ({msg_type.__name__}) "
                f"camera_id={source.camera_id} rotate_180={source.rotate_180}"
            )

        if not self.camera_sources:
            self.get_logger().warn("No camera sources configured; waiting for topic configuration.")
        ...
        self.create_timer(8.0, self.log_waiting_sources)

        if self.map_line:
            self.get_logger().info("GlobalCam ArUco map line enabled with cached 60s revalidation.")
        self.get_logger().info(f"Publishing events: {args.event_topic}")
        for source in self.camera_sources:
            if args.publish_annotated:
                self.get_logger().info(
                    f"Publishing {source.image_source} annotated image: {source.annotated_topic}"
                )
        self.get_logger().info(f"Image QoS: best_effort/depth{args.image_qos_depth}")

    def _make_image_callback(self, source: CameraSource):
        def callback(msg):
            self.on_image(msg, source)

        return callback

    def log_waiting_sources(self):
        now = time.monotonic()
        for source in self.camera_sources:
            last_msg_at = self.last_msg_at_by_source.get(source.image_source)
            if last_msg_at is None or now - last_msg_at >= 8.0:
                self.get_logger().info(
                    f"Waiting for image topic: {source.topic} source={source.image_source}",
                    throttle_duration_sec=8.0,
                )

    def on_image(self, msg: Image | CompressedImage, source: CameraSource):
        image_source = source.image_source
        source_topic = source.topic
        camera_id = source.camera_id
        self.last_msg_at_by_source[image_source] = time.monotonic()

        try:
            frame = compressed_imgmsg_to_cv(msg) if source.compressed else imgmsg_to_cv(msg)
        except Exception as exc:
            self.get_logger().warning(
                f"Image conversion failed source={image_source} topic={source_topic}: {exc}"
            )
            return

        if self.should_rotate_180(source):
        ...
        if now - last_detector_at < 1.0 / max(self.args.detector_fps, 0.1):
            ...
            if self.args.publish_annotated and last_event is not None:
                ...
                self.publish_annotated(image_source, annotated, msg)
            return
        ...
        map_line = self.update_map_line(frame, image_source)
        try:
            detections = self.detector.detect(frame)
        except Exception as exc:
            event = self.base_event(msg, frame, [], now, image_source, source_topic, camera_id, map_line)
            event.update(
                {
        ...
        self.enrich_detections_with_map(detections, map_line)
        detector_ms = (time.perf_counter() - started) * 1000.0
        event = self.build_detection_event(
            msg,
            frame,
            detections,
            now,
    ...
    def update_map_line(self, frame: np.ndarray, image_source: str):
        if image_source != "globalcam" or self.map_line is None:
            return None
        self.last_map_line = self.map_line.update(frame)
        return self.last_map_line

    def should_rotate_180(self, source: CameraSource) -> bool:
        if self.args.rotate_180:
            return True
        return source.rotate_180

    def enrich_detections_with_map(self, detections: list[DetectionResult], map_line):
        if self.map_line is None or map_line is None:
            return
        for detection in detections:
            payload = detection.to_payload()
            self.map_line.enrich_detection(payload, map_line)
            detection.map_reference_px = payload.get("map_reference_px")
            detection.map_position = payload.get("map_position")
    ...
    def build_detection_event(self, msg, frame, detections, now, detector_ms, image_source, source_topic, camera_id, map_line):
        event = self.base_event(msg, frame, detections, now, image_source, source_topic, camera_id, map_line)
        helmet_detections = [
    ...
    def publish_annotated(self, image_source: str, frame, msg):
        annotated_pub = self.annotated_pubs.get(image_source)
        if annotated_pub is None:
            return
        annotated_pub.publish(cv_to_imgmsg(frame, msg.header.frame_id, msg.header.stamp))
    ...
    def draw_overlay(self, frame, event):
        ...
        if self.map_line is not None and map_line_payload is not None:
            self.map_line.draw(annotated, map_line_payload)
        ...
        for detection in event.get("detections", []):
            ...
            if self.map_line is not None:
                self.map_line.draw_detection_reference(annotated, detection)
...
def normalize_topic(value) -> str:
    ...
    if topic in {"", "__disabled__", "disabled", "none", "None"}:
        return ""
    return topic


def resolve_global_image_topic(args) -> str:
    if args.global_image_topic is not None:
        return normalize_topic(args.global_image_topic)
    return normalize_topic(args.image_topic)


def build_camera_sources(args) -> list[CameraSource]:
    sources: list[CameraSource] = []

    global_topic = resolve_global_image_topic(args)
    if global_topic:
        sources.append(
            CameraSource(
                image_source="globalcam",
                topic=global_topic,
                compressed=args.global_image_compressed,
                rotate_180=args.global_rotate_180,
                camera_id=args.camera_id,
                annotated_topic=args.annotated_topic,
            )
        )

    robot_topic = normalize_topic(args.robot_image_topic)
    if robot_topic:
        sources.append(
            CameraSource(
                image_source="robot_picam",
                topic=robot_topic,
                compressed=args.robot_image_compressed,
                rotate_180=args.robot_rotate_180,
                camera_id=args.robot_camera_id,
                annotated_topic=args.robot_annotated_topic,
            )
        )

    robot2_topic = normalize_topic(args.robot2_image_topic)
    if robot2_topic:
        sources.append(
            CameraSource(
                image_source="robot_picam2",
                topic=robot2_topic,
                compressed=args.robot2_image_compressed,
                rotate_180=args.robot2_rotate_180,
                camera_id=args.robot2_camera_id,
                annotated_topic=args.robot2_annotated_topic,
            )
        )
...
def parse_args():
    ...
    parser.add_argument("--robot-image-topic", default="/robot/picam/image_raw", help="Optional robot Pi camera topic. Empty string disables this source.")
    parser.add_argument("--robot2-image-topic", default="", help="Optional second robot Pi camera topic. Empty string disables this source.")
    parser.add_argument("--global-image-compressed", type=parse_bool, nargs="?", const=True, default=False)
    parser.add_argument("--no-global-image-compressed", dest="global_image_compressed", action="store_false")
    parser.add_argument("--robot-image-compressed", type=parse_bool, nargs="?", const=True, default=False)
    parser.add_argument("--no-robot-image-compressed", dest="robot_image_compressed", action="store_false")
    parser.add_argument("--robot2-image-compressed", type=parse_bool, nargs="?", const=True, default=False)
    parser.add_argument("--no-robot2-image-compressed", dest="robot2_image_compressed", action="store_false")
    ...
    parser.add_argument("--robot-annotated-topic", default="/safety_identity/robot_picam_annotated_image")
    parser.add_argument("--robot2-annotated-topic", default="/safety_identity/robot_picam2_annotated_image")
    ...
    parser.add_argument("--robot2-camera-id", default="robot-picam-002")
    ...
    parser.add_argument("--registered-dir", default="/home/yr/perception_server_bundle_final/registered_faces")
    ...
    parser.add_argument("--global-rotate-180", type=parse_bool, nargs="?", const=True, default=False)
    parser.add_argument("--no-global-rotate-180", dest="global_rotate_180", action="store_false")
    parser.add_argument("--robot-rotate-180", type=parse_bool, nargs="?", const=True, default=True)
    parser.add_argument("--no-robot-rotate-180", dest="robot_rotate_180", action="store_false")
    parser.add_argument("--robot2-rotate-180", type=parse_bool, nargs="?", const=True, default=True)
    parser.add_argument("--no-robot2-rotate-180", dest="robot2_rotate_180", action="store_false")
