...
FALL_CLASS_NAMES = {"fall", "fallen_worker", "fall_detected"}
...
def bbox_xyxy_from_detection(detection: dict) -> list[float] | None:
    bbox = detection.get("bbox") or detection.get("bbox_xyxy")
...
def subject_overlap_ratio(subject: dict, other: dict) -> float:
    """Return the fraction of the subject box covered by the other box."""
    subject_bbox = bbox_xyxy_from_detection(subject)
    other_bbox = bbox_xyxy_from_detection(other)
    if subject_bbox is None or other_bbox is None:
        return 0.0
    sx1, sy1, sx2, sy2 = subject_bbox
    ox1, oy1, ox2, oy2 = other_bbox
    intersection_w = max(0.0, min(sx2, ox2) - max(sx1, ox1))
    intersection_h = max(0.0, min(sy2, oy2) - max(sy1, oy1))
    intersection_area = intersection_w * intersection_h
    subject_area = (sx2 - sx1) * (sy2 - sy1)
    return intersection_area / subject_area if subject_area > 0.0 else 0.0


def filter_falls_overlapping_turtlebots(
    safety_detections: list[dict],
    turtlebot_detections: list[dict],
    threshold: float,
) -> tuple[list[dict], list[tuple[dict, float]]]:
    if threshold <= 0.0 or not turtlebot_detections:
        return list(safety_detections), []

    kept: list[dict] = []
    suppressed: list[tuple[dict, float]] = []
    for detection in safety_detections:
        class_name = str(detection.get("class", "")).strip().lower().replace("-", "_")
        if class_name not in FALL_CLASS_NAMES:
            kept.append(detection)
            continue
        max_overlap = max(
            (subject_overlap_ratio(detection, turtlebot) for turtlebot in turtlebot_detections),
            default=0.0,
        )
        if max_overlap > 0.0 and max_overlap >= threshold:
            suppressed.append((detection, max_overlap))
        else:
            kept.append(detection)
    return kept, suppressed


def overlap_threshold(value: str) -> float:
    try:
        threshold = float(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError("overlap threshold must be a number") from exc
    if not 0.0 <= threshold <= 1.0:
        raise argparse.ArgumentTypeError("overlap threshold must be between 0.0 and 1.0")
    return threshold
...
class CocoPersonDetector:
    def __init__(self, model_path: str, confidence: float, imgsz: int, device: str):
        try:
            ...
            raise RuntimeError(
                "Person detector requires ultralytics and torch in the Python environment."
            ) from exc
...
class GlobalCamYoloResultNode(GlobalCamCombinedDetectorNode):
    def __init__(self, args):
        ...
        if args.enable_turtlebot_proximity:
            ...
            if args.enable_safety_detector:
                self.get_logger().info(
                    "Fall/TurtleBot overlap suppression enabled: "
                    f"fall_box_overlap>={args.fall_turtlebot_overlap_threshold:.2f}"
                )
    ...
    def run_safety_pipeline(self, frame, map_line, msg=None, timestamp_ns=None):
        ...
        all_detections, suppressed = filter_falls_overlapping_turtlebots(
            all_detections,
            self.last_turtlebot_detections,
            self.args.fall_turtlebot_overlap_threshold,
        )
        if suppressed:
            self.server_event_tracks.pop("fall", None)
        for detection, overlap in suppressed:
            self.get_logger().info(
                "Ignoring fall detection overlapping TurtleBot: "
                f"overlap={overlap:.3f} threshold={self.args.fall_turtlebot_overlap_threshold:.3f} "
                f"bbox={detection.get('bbox') or detection.get('bbox_xyxy')}",
    ...
    def publish_server_object_event(self, event_type: str, detection: dict, track: dict, trigger: str) -> None:
        ...
        if map_xy is None:
            return
        # Compute the approach goal before publishing the object event.  The backend
        # receives the object event first, so carrying this value in the same message
        # removes the race with /globalcam/turtlebot_goal/coordinates.
        goal_coordinate = None
        if self.map_line is not None:
            goal_coordinate = self.map_line.turtlebot_goal_position(
        ...
        if goal_coordinate is not None:
            # DB keeps payload["coordinate"] (the detected-object position); this is
            # only the robot's safe approach target for dispatch.
            payload["goal_coordinate"] = goal_coordinate
        self.server_object_event_pub.publish(String(data=json.dumps(payload, ensure_ascii=False)))
        self.publish_turtlebot_goal(detection, payload["event_id"], payload["created_at"])
        self.get_logger().info(
    ...
    def publish_turtlebot_goal(
        ...
        if self.map_line is None:
            return
        goal = self.map_line.turtlebot_goal_position(
            detection.get("map_position"),
            self.args.turtlebot_goal_offset_x,
        )
        if goal is None:
            self.get_logger().warning(
                f"Server object event {server_event_id} has no valid in-map TurtleBot goal"
            )
            return
        goal_payload = {
        ...
        self.turtlebot_goal_pub.publish(
            String(data=json.dumps(goal_payload, ensure_ascii=False))
        )
        self.get_logger().info(
            f"TurtleBot goal published for server_event_id={server_event_id} "
...
def parse_args():
    ...
    parser.add_argument("--turtlebot-goal-offset-x", type=float, default=0.6)
    ...
    parser.add_argument("--turtlebot-fps", type=float, default=4.0)
    parser.add_argument(
        "--fall-turtlebot-overlap-threshold",
        type=overlap_threshold,
        default=0.7,
    )
    ...
    parser.add_argument("--proximity-enter-distance", type=float, default=0.4)
    parser.add_argument("--proximity-exit-distance", type=float, default=0.5)
