...
class SafetyEventDetector:
    def __init__(self, model_path: str, confidence: float, imgsz: int, device: str):
        try:
            ...
            raise RuntimeError(
                "Safety event detector requires ultralytics and torch in the Python environment."
            ) from exc
...
class GlobalCamObjectMapNode(Node):
    def __init__(self, args):
        ...
        if self.map_line:
            self.get_logger().info("Fixed-corner map line enabled.")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GlobalCam safety event detection with fixed-corner map projection."
    )
