...
class TurtleBotUdpDisplayNode(Node):
    ...
    def draw_overlay(self, frame):
        ...
        # Draw server-alert banner if active
        self.draw_server_alert(annotated)
        return annotated
    ...
    def draw_server_alert(self, frame):
        ...
        if age > getattr(self.args, "alert_display_sec", 3.0):
            return

        event_type = str(self.last_server_alert.get("event_type", "server_alert")).upper()
        confidence = self.last_server_alert.get("confidence")
        if isinstance(confidence, (int, float)):
            text = f"🚨 EMERGENCY: {event_type} ({confidence*100:.1f}%)"
        else:
            text = f"🚨 EMERGENCY: {event_type}"

        # Keep the banner visible on every frame while server events continue.
        # Each new event refreshes last_server_alert_at, so the banner only clears
        # after alert_display_sec has elapsed without another server event.
        height, width = frame.shape[:2]
        ...
        # Semi-translucent red banner bar
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (width, y_pos + 15), (0, 0, 220), -1)
...
def parse_args():
    ...
    parser.add_argument("--alert-display-sec", type=float, default=1.2)
    parser.add_argument(
        "--flip-mode",
