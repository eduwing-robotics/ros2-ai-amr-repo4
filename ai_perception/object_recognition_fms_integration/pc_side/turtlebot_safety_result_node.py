...
DEFAULT_SAFETY_MODEL = str(
    Path(os.environ.get("ROBOT_FACE_SAFETY_MODEL", Path(__file__).resolve().parents[1] / "models" / "safety_continue45_plus_nohelmet_e10_best.pt"))
)
DEFAULT_PERSON_MODEL = str(Path(__file__).resolve().parents[1] / "models" / "yolo11n.pt")
EVENT_CLASS_MAP = {
    "fire": "fire",
    "fall": "fall",
    "fallen_worker": "fall",
...
INACTIVE_ROBOT_STATUSES = {
    "IDLE", "CHARGING", "RETURNING_TO_CHARGER", "RESUMING_AFTER_CHARGE",
    "LOW_BATTERY", "EMERGENCY_STOP", "PAUSED", "MANUAL_CONTROL", "DOCKING",
    "DOCK_DWELL", "UNDOCKING", "STUCK", "ESCAPE",
}
INACTIVE_ROBOT_PAUSE_REASONS = {"EVENT_PAUSED", "WAITING_PLAY", "MANUAL_PAUSED", "MANUAL_DONE"}
...
def server_event_candidate(
    ...
    if event_type == "no_helmet":
        if (
            bbox_width < args.server_head_min_width_px
            or bbox_height < args.server_head_min_height_px
        ):
            return None
    else:
        required_height = (
            args.server_fire_min_height_px
            if event_type == "fire"
            else args.server_fall_min_height_px
        )
    ...
    return {
        ...
        "detection": detection,
        "condition": {
            "center_condition": "disabled",
            "bbox_width": round(bbox_width, 1),
            "bbox_height": round(bbox_height, 1),
            "required_bbox_width": (
                args.server_head_min_width_px if event_type == "no_helmet" else None
            ),
            "required_bbox_height": (
                args.server_head_min_height_px
                if event_type == "no_helmet"
                else required_height
            ),
        },
    }
...
class FaceRecognitionWorker:
    def __init__(self, args, logger):
        ...
        # Restored 2026-07-13 path: an independent perception node publishes
        # recognized faces on /face/recognition_state.
        self.external_topic_mode = bool(args.use_external_face_result_topic)
        ...
        if not self.enabled:
            return
        if self.external_topic_mode:
            self.logger.info(f"Face recognition uses external result topic: {args.face_result_topic}")
            return
    ...
    def on_face_result_topic_received(self, payload: dict):
        """Accept the proven 2026-07-13 perception_node result stream."""
        if not self.enabled or not self.external_topic_mode:
            return
        state = str(payload.get("state", "searching")).strip().lower()
        recognized = bool(payload.get("recognized", False)) or state == "confirmed_known"
        if recognized:
            status, identity_status = "recognized", "employee"
        elif state in ("confirmed_unknown", "unknown", "no_face"):
            status, identity_status = "unknown", "unknown_candidate"
        else:
            status, identity_status = "pending", "pending"
        result = {
            "enabled": True,
            "status": status,
            "identity_status": identity_status,
            "name": payload.get("name"),
            "employee_id": payload.get("number"),
            "similarity": payload.get("best_similarity"),
            "roi_bbox_xyxy": payload.get("person_bbox"),
            "updated_at": utc_now_iso(),
            "match_method": "external_perception_node",
    ...
    @staticmethod
    def _external_result_matches_head(result: dict, head_bbox: list[int] | None) -> bool:
        person_bbox = result.get("roi_bbox_xyxy")
        if not person_bbox or not head_bbox or len(person_bbox) != 4:
            return False
        px1, py1, px2, py2 = [float(value) for value in person_bbox]
        hx1, hy1, hx2, _ = [float(value) for value in head_bbox]
        head_width = max(hx2 - hx1, 1.0)
        person_height = max(py2 - py1, 1.0)
        x_overlap = max(0.0, min(px2 + 15.0, hx2) - max(px1 - 15.0, hx1))
        return x_overlap >= head_width * 0.5 and py1 - 15.0 <= hy1 <= py1 + person_height * 0.5
    ...
    def reset_cache(self, reason: str = "reset"):
        with self.lock:
            ...
            self.last_face_completed_at = 0.0
            # A new event pause must be allowed to submit immediately.
            self.last_face_started_at = 0.0
    ...
    def process_face_roi(self, frame, detection: dict) -> dict:
        ...
        now = time.monotonic()
        if not self.external_topic_mode:
            self._submit_if_ready(frame, detection, roi_bbox, now)

        with self.lock:
            if self.last_face_result is not None and now - self.last_face_completed_at <= self.args.face_result_ttl:
                matches = (
                    self._external_result_matches_head(self.last_face_result, roi_bbox)
                    if self.external_topic_mode
                    else is_similar_face_roi(roi_bbox, self.last_face_result.get("roi_bbox_xyxy"))
                )
                if matches:
...
def detection_to_payload(
    ...
    # Restore the verified 7/13 route: per-head face results are carried on
    # /safety/detections and then copied into the corresponding server event.
...
class TurtleBotSafetyResultNode(Node):
    def __init__(self, args):
        ...
        self.face_result_sub = None
        if args.enable_face_recognition and args.use_external_face_result_topic:
            self.face_result_sub = self.create_subscription(
                String, args.face_result_topic, self.on_face_result_topic, 10
            )
            self.get_logger().info(f"Subscribed to external face result topic: {args.face_result_topic}")
        self.get_logger().info(
            "Face ROI thresholds "
        ...
        # Preserve RobotStatus-based control while keeping the updated safety pipeline.
        self.robot_status = "UNKNOWN"
        self.robot_pause_reason = ""
        # One full-frame face attempt is requested for each EVENT_NO_HELMET pause.
        self._pause_face_trigger_lock = threading.Lock()
        self._pause_face_attempt_pending = False
        self._pause_face_attempted = False
        self._pause_face_attempt_token = 0
        ...
        try:
            ...
            if robot_id is None:
                for source in (args.camera_id, args.live_topic):
                    digits = "".join(character for character in source if character.isdigit())
                    if digits:
                        robot_id = int(digits)
                        break
            status_topic = f"/robot{robot_id}/robot_status" if robot_id else "/robot_status"
            self.status_sub = self.create_subscription(RobotStatus, status_topic, self.on_robot_status, 10)
            self.get_logger().info(f"Subscribed to RobotStatus: {status_topic}")
        except Exception as exc:
            self.get_logger().warning(f"RobotStatus control unavailable; inference stays enabled: {exc}")
        ...
        self.safety_control_topic = f"/{args.camera_id}/safety/control"
        self.safety_control_sub = self.create_subscription(
            String, self.safety_control_topic, self.on_safety_control, 10
        )
        self.inference_cooldown_until = 0.0
        self.get_logger().info(f"Subscribed to safety control: {self.safety_control_topic}")
        self.annotated_pub = (
        ...
        self.get_logger().info(
            ...
            "center_condition=disabled "
            f"head_min_size={args.server_head_min_width_px}x{args.server_head_min_height_px}px "
            f"fire_min_height={args.server_fire_min_height_px}px "
            f"fall_min_height={args.server_fall_min_height_px}px "
    ...
    def on_face_result_topic(self, msg: String):
        try:
            payload = json.loads(msg.data)
            if isinstance(payload, dict):
                self.face_recognizer.on_face_result_topic_received(payload)
        except Exception as exc:
            self.get_logger().warning(f"Invalid external face result JSON: {exc}")

    def on_robot_status(self, msg):
        previous_status = self.robot_status
        previous_reason = self.robot_pause_reason
        self.robot_status = str(msg.status).strip().upper()
        self.robot_pause_reason = str(msg.pause_reason).strip().upper()
        is_no_helmet_pause = (
            self.robot_status == "PAUSED"
            and self.robot_pause_reason in ("EVENT_HELMET", "EVENT_NO_HELMET")
        )
        was_same_pause = (
            previous_status == "PAUSED"
            and previous_reason in ("EVENT_HELMET", "EVENT_NO_HELMET")
        )
        with self._pause_face_trigger_lock:
            if is_no_helmet_pause and not was_same_pause:
                self._pause_face_attempt_token += 1
                self._pause_face_attempt_pending = True
                self._pause_face_attempted = False
                self.face_recognizer.reset_cache("event_no_helmet_pause")
                self.get_logger().info("Face recognition armed for EVENT_NO_HELMET PAUSED; waiting for next frame")
            elif not is_no_helmet_pause:
                self._pause_face_attempt_token += 1
                self._pause_face_attempt_pending = False
                self._pause_face_attempted = False
        self.get_logger().info(
            f"RobotStatus status={self.robot_status} pause_reason={self.robot_pause_reason}"
        )

    def _publish_pause_face_result(self, result: dict, token: int, frame_width: int, frame_height: int):
        with self._pause_face_trigger_lock:
            if token != self._pause_face_attempt_token:
                return
        if not (
            self.robot_status == "PAUSED"
            and self.robot_pause_reason in ("EVENT_HELMET", "EVENT_NO_HELMET")
        ...
        payload = {
            ...
            "event_id": f"turtlebot_pause_face_{uuid.uuid4().hex}",
            "event_type": "no_helmet",
            "created_at": utc_now_iso(),
            "camera_id": self.args.camera_id,
            "source_topic": self.args.live_topic,
            "confidence": 0.0,
            "bbox_xyxy": [0, 0, frame_width, frame_height],
            "center_px": [frame_width / 2.0, frame_height / 2.0],
            "image_size": {"width": frame_width, "height": frame_height},
            "condition": {"trigger": "event_no_helmet_paused_full_frame"},
            "face_recognition": result,
        }
        self.server_safety_event_pub.publish(String(data=json.dumps(payload, ensure_ascii=False)))
        self.get_logger().info(
            f"PAUSED face recognition result published status={result.get('status', 'unknown')}"
        )

    def _wait_and_publish_pause_face_result(self, token: int, completed_before: float, frame_width: int, frame_height: int):
        deadline = time.monotonic() + 8.0
        while not self._stop_event.is_set() and time.monotonic() < deadline:
            with self.face_recognizer.lock:
                completed_at = self.face_recognizer.last_face_completed_at
                result = (
                    dict(self.face_recognizer.last_face_result)
                    if self.face_recognizer.last_face_result is not None
                    else None
                )
            if result is not None and completed_at > completed_before:
                self._publish_pause_face_result(result, token, frame_width, frame_height)
                return
            time.sleep(0.05)
        self.get_logger().warning("PAUSED face recognition result timed out before publish")

    def _start_pause_face_recognition(self, frame):
        with self._pause_face_trigger_lock:
            if not self._pause_face_attempt_pending or self._pause_face_attempted:
                return
            self._pause_face_attempt_pending = False
            self._pause_face_attempted = True
            token = self._pause_face_attempt_token
        frame_height, frame_width = frame.shape[:2]
        with self.face_recognizer.lock:
            completed_before = self.face_recognizer.last_face_completed_at
        initial_result = process_face_roi(
            frame,
            {"class": "face_trigger", "bbox_xyxy": [0, 0, frame_width, frame_height]},
            self.face_recognizer,
        )
        self._publish_pause_face_result(initial_result, token, frame_width, frame_height)
        initial_status = str(initial_result.get("status", "")).lower()
        if initial_status == "pending":
            threading.Thread(
                target=self._wait_and_publish_pause_face_result,
                args=(token, completed_before, frame_width, frame_height),
                name="turtlebot-paused-face-result",
                daemon=True,
            ).start()
        self.get_logger().info("PAUSED full-frame face recognition started once")

    def on_safety_control(self, msg: String):
        try:
            payload = json.loads(msg.data)
            if str(payload.get("command", "")).upper() != "COOLDOWN":
                return
            duration_sec = max(0.0, float(payload.get("duration_sec", 0.0)))
        except Exception as exc:
            self.get_logger().warning(f"Invalid safety control message: {exc}")
            return
        self.inference_cooldown_until = max(
            self.inference_cooldown_until, time.monotonic() + duration_sec
        )
        self.get_logger().warning(
            f"Safety inference cooldown enabled for {duration_sec:.1f}s reason={payload.get('reason', 'unspecified')}"
        )

    def inference_is_suspended(self) -> bool:
        if time.monotonic() < self.inference_cooldown_until:
            return True
        event_pause = self.robot_status == "PAUSED" and self.robot_pause_reason.startswith("EVENT_")
        return not event_pause and (
            self.robot_status in INACTIVE_ROBOT_STATUSES
            or self.robot_pause_reason in INACTIVE_ROBOT_PAUSE_REASONS
        )
    ...
    def on_image(self, msg: Image):
        ...
        with self._frame_lock:
            ...
            self._latest_seq += 1
        self._start_pause_face_recognition(frame)
    ...
    def _inference_loop(self):
        while not self._stop_event.is_set():
            if self.inference_is_suspended():
                frame, msg, _ = self._get_latest()
                if frame is not None and msg is not None:
                    if self.annotated_pub is not None:
                        self.annotated_pub.publish(cv_to_imgmsg(frame, msg.header.frame_id, msg.header.stamp))
    ...
    def publish_server_safety_events(self, frame, detections: list[dict]):
        ...
        for event_type, candidate in best_by_type.items():
            ...
            face_result = None
            face_attempt_allowed = (
                self.robot_status == "PAUSED"
                and self.robot_pause_reason in ("EVENT_HELMET", "EVENT_NO_HELMET")
            )
            if event_type == "no_helmet" and isinstance(detection.get("face_result"), dict):
                # 7/13 route: forward the head-matched external face result.
                face_result = dict(detection["face_result"])
            elif event_type == "no_helmet" and face_attempt_allowed:
                # Keep the current PAUSED full-frame fallback when no matched result exists yet.
                face_result = self.face_recognizer.status_result(
                    "pending", "face_recognition_triggered_on_event_pause",
                    [0, 0, frame_width, frame_height],
                )
            elif event_type == "no_helmet":
                face_result = self.face_recognizer.status_result(
                    "skipped", "face_recognition_requires_event_pause",
                    [0, 0, frame_width, frame_height],
                )
            payload = {
                "schema_version": SERVER_EVENT_SCHEMA_VERSION,
            ...
            if face_result is not None:
                payload["face_recognition"] = face_result
            self.server_safety_event_pub.publish(String(data=json.dumps(payload, ensure_ascii=False)))
            self.get_logger().info(
                "TurtleBot server safety event "
                f"event_type={event_type} "
                f"confidence={detection['confidence']:.3f} "
                f"bbox_width={condition['bbox_width']:.1f} "
...
def parse_args():
    ...
    parser.add_argument("--safety-fps", type=float, default=4.0)
    ...
    parser.add_argument("--server-head-min-width-px", type=int, default=75)
    parser.add_argument("--server-head-min-height-px", type=int, default=75)
    parser.add_argument("--server-fire-min-height-px", type=int, default=240)
    parser.add_argument("--server-fall-min-height-px", type=int, default=240)
    parser.add_argument("--server-event-cooldown-sec", type=float, default=1.0)
    ...
    parser.add_argument("--face-result-topic", default="/face/recognition_state")
    parser.add_argument("--use-external-face-result-topic", type=parse_bool, nargs="?", const=True, default=True)
    ...
    parser.add_argument("--face-temporal-votes", type=int, default=1)
    ...
    parser.add_argument(
        "--registered-dir", default=str(Path(__file__).resolve().parents[2] / "registered_faces")
    )
    parser.add_argument("--margin-threshold", type=float, default=0.03)
    parser.add_argument("--top-k", type=int, default=3)
    parser.add_argument("--face-min-roi-width", type=int, default=75)
    parser.add_argument("--face-min-roi-height", type=int, default=75)
